...
void Carol::DefaultResource::CopySubresources(RenderData* renderData, D3D12_SUBRESOURCE_DATA* subresourceData, uint32_t firstSubresource, uint32_t numSubresources)
{
	renderData->CommandList->ResourceBarrier(1,GetRvaluePtr(CD3DX12_RESOURCE_BARRIER::Transition(mResource.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST)));
	auto resourceSize = GetRequiredIntermediateSize(mResource.Get(), firstSubresource, numSubresources);
	renderData->UploadBuffersHeap->CreateResource(&mIntermediateBuffer, GetRvaluePtr(CD3DX12_RESOURCE_DESC::Buffer(resourceSize)), mIntermediateBufferAllocInfo.get(), D3D12_RESOURCE_STATE_GENERIC_READ);
	
	UpdateSubresources(renderData->CommandList, mResource.Get(), mIntermediateBuffer.Get(), 0, firstSubresource, numSubresources, subresourceData);
	renderData->CommandList->ResourceBarrier(1,GetRvaluePtr(CD3DX12_RESOURCE_BARRIER::Transition(mResource.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ)));
}
