...
{
	mRandomVecMap = std::make_unique<ColorBuffer>(
		256,
		256,
		1,
	...
	for (int i = 0; i < 256; ++i)
	{
		for (int j = 0; j < 256; ++j)
		{
			DirectX::PackedVector::XMCOLOR randVec = { rand() * 1.0f / RAND_MAX,rand() * 1.0f / RAND_MAX,rand() * 1.0f / RAND_MAX ,0.0f };
	...
	subresource.RowPitch = 256 * sizeof(DirectX::PackedVector::XMCOLOR);
