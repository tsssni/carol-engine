    ...
    case WM_COMMAND:
        {
            ...
            case ID_LOAD_MODEL:
                renderer->SetPaused(true);
                renderer->Stop();
                DialogBoxW(hInst, MAKEINTRESOURCEW(IDD_LOAD), hWnd, LoadWndProc);
            ...
            case ID_DELETE_MODEL:
                renderer->SetPaused(true);
                renderer->Stop();
                DialogBoxW(hInst, MAKEINTRESOURCEW(IDD_DELETE), hWnd, DeleteWndProc);
...
LRESULT LoadWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    static auto GetEditTextFloat = [](HWND hWnd, int resourceId, float& num, float defaultNum)
    {
        ...
        GetWindowTextW(edithWnd, buffer, 1024);
    ...
    {
    case WM_INITDIALOG:
        SetDlgItemTextW(hWnd, IDC_MODEL_NAME_EDIT, L"");
        SetDlgItemTextW(hWnd, IDC_MODEL_PATH_EDIT, L"");
        SetDlgItemTextW(hWnd, IDC_TEXTURE_PATH_EDIT, L"");
        SetDlgItemTextW(hWnd, IDC_SCALE, L"1.0");
        SetDlgItemTextW(hWnd, IDC_TRANSLATION_X, L"0.0");
        SetDlgItemTextW(hWnd, IDC_TRANSLATION_Y, L"0.0");
        SetDlgItemTextW(hWnd, IDC_TRANSLATION_Z, L"0.0");
        SetDlgItemTextW(hWnd, IDC_ROTATION, L"0.0");
        SetDlgItemTextW(hWnd, IDC_ROTATION_AXIS_X, L"0.0");
        SetDlgItemTextW(hWnd, IDC_ROTATION_AXIS_Y, L"1.0");
        SetDlgItemTextW(hWnd, IDC_ROTATION_AXIS_Z, L"0.0");
    ...
    case WM_COMMAND:
        ...
        case IDC_MODEL_PATH_BUTTON:
        {
            try
            {
                ...
                Carol::ThrowIfFailed(CoCreateInstance(CLSID_FileOpenDialog, NULL, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(pfd.GetAddressOf())));

                DWORD dwOptions;
                Carol::ThrowIfFailed(pfd->GetOptions(&dwOptions));
                pfd->SetOptions(dwOptions | FOS_FILEMUSTEXIST);

                Carol::ThrowIfFailed(pfd->Show(hWnd));
                ComPtr<IShellItem> psi;
                LPWSTR path = NULL;

                Carol::ThrowIfFailed(pfd->GetResult(psi.GetAddressOf()));
                Carol::ThrowIfFailed(psi->GetDisplayName(SIGDN_DESKTOPABSOLUTEPARSING, &path));
        ...
        case IDC_LOAD_CONFIRM:
            ...
            if (animation)
            {
                DialogBoxW(hInst, MAKEINTRESOURCEW(IDD_ANIMATION), hWnd, AnimationWndProc);
            }
...
LRESULT AnimationWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    static auto GetSelectedAnimation = [](HWND hWnd, WPARAM wParam)
    {
		...
		int lbItem = SendMessageW(lbhWnd, LB_GETCURSEL, 0, 0);

        WCHAR bufw[1024];
        SendMessageW(lbhWnd, LB_GETTEXT, lbItem, (LPARAM)bufw);
    ...
    case WM_INITDIALOG:
    {
        ...
        {
            SendMessageW(lbhWnd, LB_ADDSTRING, 0, (LPARAM)aniName.data());
        }
    }
...
LRESULT DeleteWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        {
            SendMessageW(lbhWnd, LB_ADDSTRING, 0, (LPARAM)modelName.data());
        }
    }
