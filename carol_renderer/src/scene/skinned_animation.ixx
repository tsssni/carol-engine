export module carol.renderer.scene.skinned_animation;
...
namespace Carol
{
    ...
    export class TranslationKeyframe
	{
	public:
		float TimePos;
	...
	export class RotationQuatKeyframe
	{
	public:
		float TimePos;
	...
	public:
		float GetStartTime()const
        {
            ...
            return fmin(fmin(tTrans, tScale), tQuat);
        }
		...
		float GetEndTime()const
        {
            ...
            return fmax(fmax(tTrans, tScale), tQuat);
        }

		XMVECTOR InterpolateTranslation(float t) const
        {
	    ...
	    XMVECTOR InterpolateScale(float t) const
        {
		...
		XMVECTOR InterpolateQuat(float t) const
        {
		...
		void GetFrames(vector<XMFLOAT4X4>& M)const
        {
		...
		vector<TranslationKeyframe> TranslationKeyframes;
		vector<ScaleKeyframe> ScaleKeyframes;
		vector<RotationQuatKeyframe> RotationQuatKeyframes;
	};
	...
	public:
		float GetClipStartTime()const
        {
            ...
            {
                startTime = fmin(startTime, anime.GetStartTime());
            }
            return startTime;
        }
		...
		float GetClipEndTime()const
        {
            ...
            for (auto anime : BoneAnimations)
            {
                endTime = fmax(endTime, anime.GetEndTime());
            }
            return endTime;
        }
		...
		void GetCriticalFrames(vector<vector<XMFLOAT4X4>>& frames)const
        {
